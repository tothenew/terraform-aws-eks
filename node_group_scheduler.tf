resource "aws_autoscaling_schedule" "eks_stop" {
  ...
  count = var.enable_schedule ? length(compact([for group in module.self_managed_node_group : group.autoscaling_group_name])): 0
  scheduled_action_name  = "self-managed-nodegroup-${var.cluster_name}-stop"
  min_size               = 0
  max_size               = 0
  desired_capacity       = 0
  autoscaling_group_name = compact([for group in module.self_managed_node_group : group.autoscaling_group_name])[count.index]
